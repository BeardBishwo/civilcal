// Resolve BASE URL dynamically (from manifest or global)
const BASE = (() => {
    try {
        const m = document.querySelector('link[rel="manifest"]');
        if (m && m.href) {
            return m.href.replace(/\/manifest\.json.*/, '');
        }
    } catch (e) {}
    return (typeof window !== 'undefined' && window.APP_BASE) ? String(window.APP_BASE).replace(/\/$/, '') : '';
})();

// Catalog data: categories -> subcategories -> tools
async function loadDefaultCatalog() {
    try {
        const response = await fetch(`${BASE}/db/catalog.json`);
...
function showCalculator(calculatorId){
    ...
    if (!el) {
        ...
        window.location.href = `${BASE}/${calculatorId}.php`;
        return;
    }
...
function saveToHistory(calculator, calculation, result){
    ...
    if (navigator.onLine) {
        fetch(`${BASE}/api/save_history.php`, {
            method: 'POST',
