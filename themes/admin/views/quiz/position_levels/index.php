...
<script>
    ...
    async function saveLevel() {
        ...
        try {
            const response = await fetch('<?php echo app_base_url('admin/quiz/position-levels/store'); ?>', {
                method: 'POST',
                body: formData
            });
    ...
    function deleteLevel(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const res = await fetch('<?php echo app_base_url('admin/quiz/position-levels/delete/'); ?>' + id, {
                    method: 'POST'
                });
...
<style>
    ...
    .nav-ball-container.active .nav-ball-toggle {
    ...
    .nav-ball-links::-webkit-scrollbar {
        width: 5px;
    }

    .nav-ball-links::-webkit-scrollbar-thumb {
        background: #e2e8f0;
        border-radius: 10px;
    }
