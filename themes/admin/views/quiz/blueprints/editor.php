        ...
        <div class="modal-content">
            ...
            <form id="ruleForm" onsubmit="saveRule(event)">
                <div class="modal-body">
                    ...
                    <div class="arch-input-group inverted">
                        <label>Difficulty Balancing (E / E-M / M / H / EX)</label>
                        <div class="triple-input" style="grid-template-columns: repeat(5, 1fr);">
                            <input type="number" id="difficultyEasy" placeholder="0" title="Easy">
                            <input type="number" id="difficultyEasyMid" placeholder="0" title="Easy-Mid">
                            <input type="number" id="difficultyMedium" placeholder="0" title="Medium">
                            <input type="number" id="difficultyHard" placeholder="0" title="Hard">
                            <input type="number" id="difficultyExpert" placeholder="0" title="Expert">
                        </div>
                    </div>
                </div>
...
<style>
   ...
   ======================================== */
    ...
    .arch-input-group label {
        font-size: 0.75rem;
        font-weight: 700;
        color: var(--arch-text-main);
    }

    .arch-input-group input,
    .arch-input-group select,
    .arch-input-group textarea {
        width: 100%;
    ...
    .arch-modal .modal-header {
        background: var(--arch-dark);
        border: none;
        padding: 1.5rem 2rem;
    }

    .arch-modal .modal-title {
        color: white;
        font-weight: 800;
        font-size: 1.1rem;
    }

    .arch-modal .modal-body {
        padding: 2rem;
    }

    .arch-modal .modal-footer {
        padding: 0 2rem 2rem;
        border: none;
    }

    .arch-input-group.inverted label {
        color: var(--arch-text-muted);
        font-weight: 800;
        text-transform: uppercase;
        font-size: 0.7rem;
    ...
    .arch-input-group.inverted input,
    .arch-input-group.inverted select {
        background: white;
        border-color: #cbd5e1;
    }
    ...
    input:checked+.arch-slider {
        background: var(--arch-success);
    }

    input:checked+.arch-slider:before {
...
<script>
    ...
    async function saveBlueprint() {
        ...
        try {
            ...
            Swal.fire('Error', 'Server Sync Failure', 'error');
        }
    }
    ...
    async function saveRule(event) {
        ...
        const easyMid = parseInt(document.getElementById('difficultyEasyMid').value) || 0;
        ...
        const expert = parseInt(document.getElementById('difficultyExpert').value) || 0;

        if (easy > 0) difficultyDist.easy = easy;
        if (easyMid > 0) difficultyDist.easy_mid = easyMid;
        if (medium > 0) difficultyDist.medium = medium;
        if (hard > 0) difficultyDist.hard = hard;
        if (expert > 0) difficultyDist.expert = expert;
        ...
        try {
            ...
            else Swal.fire('Validation Error', res.error, 'error');
        } catch (e) {
    ...
    async function deleteRule(ruleId) {
        const res = await Swal.fire({
            title: 'Purge Rule?',
            text: "This will remove this syllabus mapping from the template.",
            icon: 'warning',
        ...
        try {
            const r = await fetch(`${baseUrl}/admin/quiz/blueprints/rule/${ruleId}/delete`, {
                method: 'POST'
            });
    ...
    async function confirmGenerate(event) {
        ...
        Swal.fire({
            title: 'Executing Engine...',
            html: 'Selecting optimal question paths...',
        ...
        try {
            ...
            Swal.fire('Error', 'Fatal Core Error', 'error');
        }
    }
...
function renderRuleRow($rule)
{
    ...
    if (!empty($diffDist)) {
        foreach ($diffDist as $level => $count) {
            $label = str_replace('_', '-', strtoupper($level));
            $html .= '<span class="arch-diff-badge arch-diff-' . $level . '">' . $label . ': ' . $count . '</span>';
        }
    } else {
