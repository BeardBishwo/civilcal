...
const AdminApp = {
  ...
  initSidebar() {
    ...
    const syncSidebarState = (shouldCollapse, persist = true) => {
      ...
      if (mainContent) {
        ...
        // Force charts and other elements to resize (without flicker)
  ...
  setupSubmenus(sidebar) {
    ...
    ).filter((item) => item.querySelector(".nav-submenu"));
      ...
      if (!link.dataset.submenuInit) {
        ...
        link.addEventListener("click", (event) => {
          ...
          this.submenuItems.forEach((other) => {
            this.setSubmenuExpanded(
              other,
              other === item ? shouldExpand : false,
              sidebar
            );
          });
        });
    ...
    const hasServerActive = submenuItems.some((item) =>
  ...
  syncActiveSubmenuDisplay() {
    ...
    const items = this.submenuItems.length
      ? this.submenuItems
      : Array.from(document.querySelectorAll(".nav-item")).filter((item) =>
          item.querySelector(".nav-submenu")
        );
  ...
  initializeDashboardCharts() {
    ...
    const calculatorUsageCanvas = document.getElementById(
      "calculatorUsageChart"
    );
...
async function createBackup() {
  ...
  try {
    ...
    if (result.success) {
      AdminApp.showNotification(
        `Backup created: ${result.backup_name} (${result.file_size})`,
        "success"
      );
    } else {
...
window.showToast = function (type = "info", titleOrMessage = "", message = "") {
  try {
    ...
    if (
      window.AdminApp &&
      typeof window.AdminApp.showNotification === "function"
    ) {
    ...
    if (
      window.ProCalculator &&
      typeof window.ProCalculator.showNotification === "function"
    ) {
