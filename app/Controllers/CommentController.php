...
class CommentController extends Controller
{
    ...
    private function requireCsrfToken()
    {
        ...
        if (empty($token) || !\App\Services\Security::validateCsrfToken($token)) {
    ...
    public function vote($commentId)
    {
        ...
        try {
            ...
            if ($result) {
                ...
                $userVote = $this->voteModel->getUserVoteForComment($commentId, Auth::id());
    ...
    public function update($commentId)
    {
        ...
        // No strict CSRF check for PUT? Prompt asked for "create, vote, update, delete, report".
        // Usually PUT requests also need CSRF.
        $this->requireCsrfToken();
    ...
    public function delete($commentId)
    {
        ...
        // DELETE method also needs checks
        $this->requireCsrfToken();
    ...
    public function report($commentId)
    {
        ...
        try {
            ...
            $existingReport = $this->commentModel->getUserReportForComment($commentId, Auth::id());
            if ($existingReport) {
            ...
            // Create report
            $reportId = $this->commentModel->reportComment($commentId, Auth::id(), $reason, $description);
    ...
    public function userComments($userId)
    {
        try {
            ...
            if (Auth::check() && Auth::id() == $userId) {
                // Should we require auth here? The original code had $this->requireAuth() if checking own history?
                // The prompt says "standardize auth usage".
                // I'll leave the logic as "if authenticated and same user, proceed". The auth is established by Auth::check().
            }
