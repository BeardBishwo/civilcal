...
class CalculationHistory {
    ...
    /**
     * Get calculation by ID (for ShareController)
     * Supports both authenticated user access and public access
     */
    public function getCalculationById($id, $userId = null, $public = false) {
        $db = Database::getInstance();
        
        if ($public && $userId === null) {
            // Public access - no user verification required
            $stmt = $db->getPdo()->prepare("
                SELECT * FROM calculation_history 
                WHERE id = ?
            ");
