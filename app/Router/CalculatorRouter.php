...
class CalculatorRouter
{
    ...
    public function handleRequest($route)
    {
        ...
        // Performance Optimization: Quickly skip standard application routes
        $standardRoutes = ['login', 'register', 'dashboard', 'logout', 'profile', 'settings', 'admin', 'api', 'quiz'];
        $firstPart = explode('/', $route)[0];
        if (in_array($firstPart, $standardRoutes)) {
            return false;
        }
        ...
        try {
            ...
            throw new \Exception("DB Error in Router: " . $e->getMessage() . " - DB: " . $this->db->prepare('SELECT DATABASE()')->execute() ? $this->db->prepare('SELECT DATABASE()')->fetchColumn() : 'unknown');
        }
