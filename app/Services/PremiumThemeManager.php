...
class PremiumThemeManager
{
    ...
    private function shouldValidateRemotely($license)
    {
        ...
        $lastValidation = strtotime($license['last_validated_at'] ?? $license['created_at']);
        $daysSinceValidation = $lastValidation ? floor((time() - $lastValidation) / (60 * 60 * 24)) : 999;
    ...
    public function getActiveLicense()
    {
        ...
        $stmt = $this->database->prepare("
            SELECT * FROM theme_licenses 
            WHERE active = 1 
            AND (allowed_domains LIKE ? OR allowed_domains LIKE ? OR allowed_domains LIKE ?) 
            ORDER BY created_at DESC 
            LIMIT 1
        ");
        $stmt->execute(["%{$domain}%", "%*%", "%localhost%"]);
     ...
     * @return int
     */
    public function getAnalyticsCount($themeName, $eventType)
    {
        if (!$this->database) {
            return 0;
        }
        ...
        $stmt = $this->database->prepare("
            ...
            WHERE theme_name = ? AND event_type = ?
        ");
        $stmt->execute([$themeName, $eventType]);
     ...
     * @return array
     */
    public function getRecentAnalytics($limit = 10)
    {
        if (!$this->database) {
            return [];
        }
     ...
     */
    public function logAnalytics($themeName, $eventType, $eventData = [])
    {
        if (!$this->database) {
            return;
        }
        ...
        $stmt = $this->database->prepare("
            INSERT INTO theme_analytics (theme_name, user_id, event_type, event_data, ip_address, domain) 
            VALUES (?, ?, ?, ?, ?, ?)
        ");
     ...
     * @return array
     */
    private function validateRemoteLicense($licenseKey, $domain = null)
    {
        // For now, return success - implement actual remote validation as needed
        return [
            'valid' => true,
            'message' => 'Remote validation skipped in demo'
        ];
    }
}
