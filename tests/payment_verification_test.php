...
// Manually include PayTMLibrary if not in autoload
if (!class_exists('PayTMLibrary') && file_exists('app/Libraries/PayTMLibrary.php')) {
    require_once 'app/Libraries/PayTMLibrary.php';
}
...
// Test 6: Check Settings Model
echo "\n6. Testing Settings Model...\n";
if (class_exists('App\\Models\\Settings')) {
    echo "   ✅ Settings model found\n";
    try {
        $settings = new App\Models\Settings();
        if (method_exists($settings, 'getPaymentSettings')) {
            echo "   ✅ getPaymentSettings method exists\n";
        } else {
            echo "   ⚠️  getPaymentSettings method not found\n";
        }
    } catch (Exception $e) {
        echo "   ❌ Settings model error: " . $e->getMessage() . "\n";
    }
} else {
