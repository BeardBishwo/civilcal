...
try {
    ...
    ";
    $pdo->exec($sql1);
    echo "Table 'bot_profiles' created/verified.\n";
    ...
    CREATE TABLE IF NOT EXISTS quiz_lobbies (
        ...
        exam_id INT UNSIGNED NOT NULL, -- Match tables
        host_user_id INT UNSIGNED DEFAULT NULL, -- Match tables
    ...
    ";
    $pdo->exec($sql2);
    echo "Table 'quiz_lobbies' created/verified.\n";
    ...
    CREATE TABLE IF NOT EXISTS quiz_lobby_participants (
        ...
        user_id INT UNSIGNED DEFAULT NULL, -- Match users table
        bot_profile_id BIGINT UNSIGNED DEFAULT NULL, -- Match bot_profiles
        ...
        -- Removed FK on user_id to avoid strict dependency on users table if name differs, though likely 'users'
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
    ";
    $pdo->exec($sql3);
    echo "Table 'quiz_lobby_participants' created/verified.\n";

    // 4. Seeder
    $stmt = $pdo->query("SELECT COUNT(*) FROM bot_profiles");
    if ($stmt->fetchColumn() == 0) {
        echo "Seeding Bot Profiles...\n";
        $bots = [
