...
<head>
    ...
    <title>Water Pipe Sizing - AEC Calculator</title>
...
<body>
    <div class="container">
        <div class="calculator-wrapper">
            <h2 class="text-center mb-4">
                <i class="fas fa-pipe me-2"></i>Water Pipe Sizing
            </h2>
            
            <form id="water-pipe-form">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div id="flow-details-section" class="section-content mb-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="flowRate" min="0" step="0.1" value="2">
                            </div>

                            <div class="form-group">
                                <label for="velocity">Target Velocity (m/s)</label>
                            ...
                            <div id="reference-section" class="section-content">
                                <table class="reference-table">
                                    <thead>
                                        <tr>
                                            <th>Size (mm)</th>
                                            <th>Flow @ 1.5m/s (L/s)</th>
                                            <th>Flow @ 2.0m/s (L/s)</th>
                                        </tr>
                                    </thead>
                                    <tbody id="flowTable">
                                    </tbody>
    ...
    <script>
        ...
        document.getElementById('water-pipe-form').addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            resultText += `Flow Rate: ${flowRate} L/s<br>`;
            resultText += `Target Velocity: ${velocity} m/s<br><br>`;
            ...
            saveRecent(`${flowRate}L/s @ ${velocity}m/s → ${results.recommendedSize}mm`);
        });

        function calculatePipeSize(flow, velocity, material) {
            const warnings = [];
            ...
            const recommendedSize = standardSizes.find(s => s >= diameter) || 100;
            
            const actualArea = Math.PI * Math.pow(recommendedSize / 2000, 2);
            const actualVelocity = flowM3s / actualArea;
            
            if (actualVelocity > 2.5) {
                warnings.push('High velocity - risk of noise and erosion');
            }
            
            if (actualVelocity < 0.5) {
                warnings.push('Low velocity - risk of sedimentation');
            }
        ...
        function populateTable() {
            ...
            table.innerHTML = standardSizes.map(size => {
                const area = Math.PI * Math.pow(size / 2000, 2);
                const flow1_5 = (area * 1.5 * 1000).toFixed(2);
                const flow2_0 = (area * 2.0 * 1000).toFixed(2);
                return `
                    <tr>
                        <td>${size}</td>
                        <td>${flow1_5}</td>
        ...
        function saveRecent(calculation) {
            const key = 'recentWaterPipeCalculations';
            let recent = JSON.parse(localStorage.getItem(key) || '[]');
            recent.unshift({
                type: 'Water Pipe Sizing',
        ...
        // Initialize
        document.getElementById('flow-details-section').style.display = 'block';
